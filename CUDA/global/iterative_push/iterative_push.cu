...
__device__ void randomMove(myCurandState_t *state,int grid[][SIZE+2], int new_grid[][SIZE+2]){
	...
	}while( atomicCAS(&new_grid[row][column],0,grid[idx][idy])    );
...
__global__ void initCurand(myCurandState_t state[][SIZE+2], unsigned long seed){
	...
	int idy=blockIdx.y*blockDim.y+threadIdx.y;
	curand_init( 0 ,idx*(SIZE+2)+idy, 0, &state[idx][idy]);

}



__global__ void compute(int grid[][SIZE+2], int new_grid[][SIZE+2], int temp_grid[][SIZE+2],int move_grid[][SIZE+2] ){
	...
	if(temp_grid[idx][idy] != 0 ){
		...
		while(true) {
			...
			if(new_grid[row][column] == 0){
				...
				//Attempt to find an emply cell or displace agent with lower priority		
				old_value = atomicMax(&move_grid[row][column],current_priority);

				if(old_value < current_priority){ 

					if(old_value != 0){ //Displaced another agent 
						agentsLeft = true;
				...
				} else { //Failed to displace left to next iteration
					agentsLeft = true;
				}
...
__global__ void updateTonew (int grid[][SIZE+2], int new_grid[][SIZE+2],int temp_grid[][SIZE+2],int move_grid[][SIZE+2],int rowAndColumn[][SIZE+2]){
    int idx=blockIdx.x*blockDim.x+threadIdx.x;
    int idy=blockIdx.y*blockDim.y+threadIdx.y;
	
    int movegrid_priority = move_grid[idx][idy];


    if(movegrid_priority != 0){ //Resource is slected by an agent 
	
    	int source_row = movegrid_priority / (SIZE + 2);
    	int source_col = movegrid_priority % (SIZE + 2);
...
int main(int argc, char* argv[])
{
	...
	for(int i=0; i<numRoundsTotal; i++){ //Simulation cycles
		...
		//Compute Happiness
		compute << <gridSize, blockSize >> >(device_grid, device_newGrid,device_tempGrid, device_moveGrid); 
		...
		do{
			...
			updateTonew << <gridSize, blockSize >> >(device_grid, device_newGrid,device_tempGrid,device_moveGrid,device_rowAndColumn);
