...
__global__ void updateTonew (int grid[][SIZE+2], int new_grid[][SIZE+2],int temp_grid[][SIZE+2],int move_grid[][SIZE+2], int rowAndColumn[][SIZE+2], myCurandState_t state[][SIZE + 2]){
    ...
    if(!idx || !idy || idx > SIZE || idy > SIZE)
        return;
  
    int neighbourhood = cell_neighbourhood;
	
    for(int dx = -neighbourhood / 2; dx <= neighbourhood / 2; dx++) {
        for(int dy = -neighbourhood / 2; dy <= neighbourhood / 2; dy++) {
            if(idx + dx < 1 || idx + dx > SIZE || idy + dy < 1 || idy + dy > SIZE)
                continue;

            int val = move_grid[idx + dx][idy + dy];
            if(val == cell_id) {
                candidates[num_candidates] = (idx + dx) * (SIZE + 2) + (idy + dy);
                num_candidates++;
...
int main(int argc, char* argv[])
{
	...
	for(int i=0; i<numRoundsTotal; i++){ //Simulation cycles
		...
		do{
			...
			updateTonew << <gridSize, blockSize >> >(device_grid, device_newGrid,device_tempGrid,device_moveGrid, device_rowAndColumn, devState);
