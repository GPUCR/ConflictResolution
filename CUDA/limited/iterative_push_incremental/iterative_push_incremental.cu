...
__global__ void assign(myCurandState_t *state, int grid[][SIZE + 2],
                       ...
                       int move_grid[][SIZE + 2],
                       int rowAndColumn[][SIZE + 2], int permutation[][PESIZE], int permutation_inverse[][SIZE])
{
    ...
    int local_neighbourhood = limitedNeighbourhood;
    int loopCounter = 0;
    ...
    if (temp_grid[idx][idy] != 0) {
        do {
            ...
            if(loopCounter >= 10) {
                loopCounter = 0;
                local_neighbourhood += 3;
            }


            int randomRow = (getnextrandint(&state[idx * (SIZE + 2) + idy]) % local_neighbourhood) - (local_neighbourhood / 2);
            row = idx + randomRow;
 
            int randomColumn = (getnextrandint(&state[idx * (SIZE + 2) + idy]) % local_neighbourhood) - (local_neighbourhood / 2);
